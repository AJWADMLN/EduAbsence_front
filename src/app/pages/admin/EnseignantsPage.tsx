...
import {
  ...
  AlertTriangle, Building2, Hash, History, ArrowLeft, Clock, Calendar,
} from "lucide-react";
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl border border-gray-100 overflow-hidden my-4">
        ...
        <div className="bg-gradient-to-r from-blue-600 to-indigo-600 px-6 py-5">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              <div className={`w-12 h-12 rounded-2xl flex items-center justify-center text-sm flex-shrink-0 ${isMasc ? "bg-blue-400/30" : "bg-pink-300/30"}`} style={{ fontWeight: 700, fontSize: "1.1rem", color: "white" }}>
                {enseignant.prenom.charAt(0)}{enseignant.nom.charAt(0)}
              </div>
              <div>
                <p className="text-white" style={{ fontWeight: 700, fontSize: "1.05rem" }}>
                ...
                <div className="flex items-center gap-2 flex-wrap mt-0.5">
                  ...
                  <span className="text-blue-100 text-xs flex items-center gap-1">
                    <Hash className="w-3 h-3" />{enseignant.ppr}
                  </span>
                </div>
              </div>
            </div>
        ...
        <div className="p-6">
          ...
          <div className="flex items-center gap-2 bg-blue-50 rounded-xl px-4 py-2.5 mb-5">
            ...
            <span className="text-blue-800 text-sm" style={{ fontWeight: 500 }}>
              {eta?.nom || `Établissement #${enseignant.etaId}`}
            </span>
          ...
          {enseignant.totalHeureAbsences > 0 && (
            <div className="flex items-center gap-2 bg-red-50 rounded-xl px-4 py-2.5 mb-5">
              ...
              <span className="text-red-800 text-sm" style={{ fontWeight: 600 }}>
                {enseignant.totalHeureAbsences}h d'absences irrégulières cumulées
              </span>
            </div>
          )}
          ...
          {jours.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-12 text-gray-400">
              ...
              <p className="text-sm mt-1">Cet enseignant n'a jamais été absent.</p>
            </div>
          ) : (
              ...
              {jours.map(({ date, heuresTotal, quarts }) => (
                <div key={date} className="flex items-center gap-4 p-4 bg-gray-50 rounded-2xl border border-gray-100 hover:border-blue-200 transition">
                  <div className="flex-shrink-0 text-center w-16">
                    <p className="text-xs text-gray-400 uppercase" style={{ fontWeight: 600 }}>
                      {new Date(date).toLocaleDateString("fr-FR", { weekday: "short" })}
                    </p>
                    <p className="text-gray-800 text-lg leading-none" style={{ fontWeight: 800 }}>
                  ...
                  <div className="flex-1 flex flex-wrap gap-2">
                    {quarts.map((q, i) => {
                      ...
                      return (
                        <span
                          key={i}
                          className={`inline-flex items-center gap-1.5 text-xs px-3 py-1.5 rounded-xl ${isMatin ? "bg-amber-100 text-amber-700" : q.quart === "soir" ? "bg-indigo-100 text-indigo-700" : "bg-gray-100 text-gray-600"}`}
                    ...
                    <span className="inline-flex items-center text-xs px-2 py-1 rounded-full bg-red-50 text-red-600 border border-red-100" style={{ fontWeight: 500 }}>
                      Irrégulière
                    </span>
                  </div>
            ...
            {jours.length > 0 && (
              <div className="ml-auto flex items-center gap-2 bg-blue-50 rounded-xl px-4 py-2">
                ...
                <span className="text-blue-700 text-sm" style={{ fontWeight: 700 }}>
                  {jours.reduce((s, j) => s + j.heuresTotal, 0)}h
                </span>
...
export default function EnseignantsPage() {
  const { enseignants, absences, etablissements, addEnseignant, updateEnseignant, deleteEnseignant, currentUser } = useApp();
  ...
  const getEtaNom = (etaId: number) => etablissements.find(e => e.id === etaId)?.nom || `Éta. #${etaId}`;

  const filtered = enseignants.filter(e =>
    `${e.nom} ${e.prenom} ${e.matiere || ""} ${e.cycle} ${e.ppr} ${getEtaNom(e.etaId)}`
      .toLowerCase().includes(search.toLowerCase())
  );

  const paginated = filtered.slice((page - 1) * PER_PAGE, page * PER_PAGE);

  const handleSave = (data: Omit<Enseignant, "totalHeureAbsences">) => {
    if (editTarget) updateEnseignant(editTarget.ppr, data);
    else addEnseignant(data);
  };

  const handleEdit   = (e: Enseignant) => { setEditTarget(e); setModalOpen(true); };
  const handleNew    = () => { setEditTarget(null); setModalOpen(true); };
  const handleDelete = () => { if (deleteTarget) { deleteEnseignant(deleteTarget.ppr); setDeleteTarget(null); } };

  const absCount = (ppr: number) => absences.filter(a => a.enseignantPpr === ppr).length;

  return (
    <div className="space-y-5">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h2 className="text-gray-900" style={{ fontWeight: 700 }}>Liste des enseignants</h2>
          <p className="text-gray-500 text-sm">{enseignants.length} enseignant{enseignants.length > 1 ? "s" : ""} enregistré{enseignants.length > 1 ? "s" : ""}</p>
        </div>
        {!isConsultant && (
              ...
              <table className="w-full">
                <thead>
                  <tr className="bg-gray-50 border-b border-gray-100">
                    ...
                    <th className="text-left text-xs text-gray-500 px-5 py-3" style={{ fontWeight: 600 }}>Cycle & Établissement</th>
                    <th className="text-left text-xs text-gray-500 px-5 py-3" style={{ fontWeight: 600 }}>Absences (h)</th>
                    <th className="text-right text-xs text-gray-500 px-5 py-3" style={{ fontWeight: 600 }}>Actions</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-gray-50">
                  {paginated.map(e => {
                    const count = absCount(e.ppr);
                    return (
                      <tr key={e.ppr} className="hover:bg-gray-50/50 transition">
                        ...
                        <td className="px-5 py-3.5">
                          <span className="flex items-center gap-1 text-xs text-gray-600 bg-gray-100 px-2.5 py-1 rounded-full w-fit" style={{ fontWeight: 600 }}>
                            ...
                            {e.ppr || <span className="text-gray-400 italic">—</span>}
                          </span>
                        </td>
                        <td className="px-5 py-3.5">
                          <span className="text-sm text-gray-600 bg-gray-100 px-2.5 py-1 rounded-full" style={{ fontWeight: 500 }}>
                            {e.matiere || "—"}
                          </span>
                        </td>
                        <td className="px-5 py-3.5">
                          <div className="space-y-1">
                            <span className={`text-xs px-2 py-0.5 rounded-full inline-block ${
                              e.cycle === "Primaire" ? "bg-green-100 text-green-700" :
                              e.cycle === "Collège"  ? "bg-indigo-100 text-indigo-700" :
                            ...
                            <div className="flex items-center gap-1 text-xs text-gray-500">
                              ...
                              <span className="truncate max-w-[140px]">{getEtaNom(e.etaId)}</span>
                            </div>
                          </div>
                        </td>
                        <td className="px-5 py-3.5">
                          <span className={`text-sm px-2.5 py-1 rounded-full ${e.totalHeureAbsences === 0 ? "bg-green-100 text-green-700" : e.totalHeureAbsences <= 6 ? "bg-amber-100 text-amber-700" : "bg-red-100 text-red-700"}`} style={{ fontWeight: 600 }}>
                            {e.totalHeureAbsences}h
                          </span>
                        </td>
                        <td className="px-5 py-3.5">
                          <div className="flex items-center justify-end gap-1">
                            <button onClick={() => setHistTarget(e)} className="p-2 text-indigo-500 hover:bg-indigo-50 rounded-lg transition" title="Historique des absences">
                              <History className="w-4 h-4" />
                            </button>
                            {!isConsultant && (
                              <>
                                <button onClick={() => handleEdit(e)} className="p-2 text-blue-500 hover:bg-blue-50 rounded-lg transition" title="Modifier">
                                  <Edit2 className="w-4 h-4" />
                                </button>
                                <button onClick={() => setDeleteTarget(e)} className="p-2 text-red-500 hover:bg-red-50 rounded-lg transition" title="Supprimer">
                ...
                return (
                  <div key={e.ppr} className="p-4">
                        ...
                        <div>
                          ...
                          <div className="flex items-center gap-1.5 flex-wrap mt-0.5">
                            ...
                            <span className={`text-xs px-2 py-0.5 rounded-full ${
                              ...
                              e.cycle === "Collège"  ? "bg-indigo-100 text-indigo-700" :
                              "bg-purple-100 text-purple-700"
                            }`} style={{ fontWeight: 500 }}>{e.cycle}</span>
                          </div>
                        </div>
                      </div>
                      <div className="flex gap-1">
                        <button onClick={() => setHistTarget(e)} className="p-2 text-indigo-500 hover:bg-indigo-50 rounded-lg" title="Historique"><History className="w-4 h-4" /></button>
                        {!isConsultant && (
                          <>
                            <button onClick={() => handleEdit(e)} className="p-2 text-blue-500 hover:bg-blue-50 rounded-lg"><Edit2 className="w-4 h-4" /></button>
                            <button onClick={() => setDeleteTarget(e)} className="p-2 text-red-500 hover:bg-red-50 rounded-lg"><Trash2 className="w-4 h-4" /></button>
                          </>
                        )}
                      </div>
                    </div>
                    <div className="mt-2 flex items-center gap-3 text-xs text-gray-500 flex-wrap">
                      ...
                      <span className="flex items-center gap-1"><Building2 className="w-3 h-3" />{getEtaNom(e.etaId)}</span>
                      <span className={`px-2 py-0.5 rounded-full ${count === 0 ? "bg-green-100 text-green-700" : "bg-red-100 text-red-700"}`} style={{ fontWeight: 600 }}>
            ...
            <div className="flex flex-col items-center text-center">
              ...
              <div className="flex gap-3 w-full">
                ...
                <button onClick={handleDelete}
                  className="flex-1 bg-red-600 hover:bg-red-700 text-white py-2.5 rounded-xl transition text-sm" style={{ fontWeight: 600 }}>
                  Supprimer
                </button>
              </div>
            </div>
